...
export function createTaskService(
  ...
  return {
    ...
    listTasks(user: PublicUser | null | undefined, projectId: string, options: ListTasksOptions = {}) {
      ...
      // Một lần đọc repository, sau đó lọc/sắp xếp trong cùng tập dữ liệu: không có N+1 query.
      // Chỉ gọi repository một lần, nên endpoint này không bị N+1 query.
    ...
    async updateTask(user: PublicUser | null | undefined, taskId: string, changes: CreateTaskInput): Promise<Task> {
      if (!user) throw new Error('Chưa đăng nhập');
