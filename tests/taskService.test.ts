...
describe('dịch vụ task', () => {
  it('giữ cách tính điểm ưu tiên cũ', () => {
    expect(calculateTaskPriorityScore({ priority: 5, dueInDays: 0, blocked: true })).toBe(120);
  });

  it('trả trang cursor ổn định với cùng giá trị sort', async () => {
    ...
    await service.createTask(user, { projectId: 'p1', title: 'Một' });
    await service.createTask(user, { projectId: 'p1', title: 'Hai' });
