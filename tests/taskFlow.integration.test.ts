...
describe('TaskFlow integration', () => {
  it('paginates, filters, and sorts tasks without duplicates', async () => {
    const app = createApp();
    const token = await registerAndLogin(app, 'owner@test.com');
    const project = await request(app)
      .post('/projects')
      .set('Authorization', `Bearer ${token}`)
      .send({ name: 'Performance', workspaceId: 'workspace-1' })
      .expect(201);
  ...
  it('caches workspace data and invalidates stats after task changes', async () => {
